...
def FitModels(x_train,y_train,x_test=None,y_test=None,createTest=False):
    ...
    testModel(x_train,y_train,x_test,y_test,
              Forest(tree_method='naive',n_estimators=1,n_jobs=8,max_depth=3, min_samples_split=30), 
                            Forest(tree_method='array',n_estimators=1,n_jobs=8,max_depth=3, min_samples_split=30), name="RF")

def testModel(x_train,y_train,x_test,y_test,model,arrayModel,name):
    #fitting
    print(f"Fitting model {name}")
    model.fit(x_train,y_train) #train
    arrayModel.fit(x_train, y_train, model.custom_trees)

    #testing
    print(f"Testing model {name} naive")
    ...
    print(f"Confusion Matrix: \n {confusion}")
     ...
     #testing
    print(f"Testing model {name} array")
    start = timeit.default_timer()
    y_pred = arrayModel.predict(x_test)
    end = timeit.default_timer()
